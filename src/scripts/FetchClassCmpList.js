...
class ClassCmpListFetcher {

    async main(orgName, toFetch) {
    ...
    async fetchAuraComponentsList(orgName, conn) {
        try {
            ...
            let outputFile = `${ROOT}/FetchedClassCmpList/`;
            fs?.mkdirSync(outputFile, { recursive: true });
            outputFile += `${orgName}_aura.txt`;
            let suffix = Utils.aura_suffixMap;
            let auraList = Array.from(auraRecords).filter(y => y['DefType'] in suffix).map(x => {
    ...
    async fetchApexClassesList(orgName, conn) {
        try {
            ...
            let apexList = Array.from(apexRecords).map(x => x['Name']);
    ...
    async fetchLightningWebComponentList(orgName, conn) {
        try {
            ...
            let lwcList = Array.from(lwcRecords).map(x => x['FilePath']);
    ...
    async fetchVisualforcePageList(orgName, conn) {
        try {
            ...
            let vfList = Array.from(vfRecords).map(x => x['Name']);
    ...
    async fetchVisualforceCmpList(orgName, conn) {
        try {
...
module.exports = ClassCmpListFetcher;

/* ENTRY POINT */
if(require.main === module)
    new ClassCmpListFetcher().main(ORG_NAME);
/* ! ENTRY POINT */
