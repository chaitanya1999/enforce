...
class CodeFetcher {
    ...
    async main(codeToFetchParam, delFlag, enForceMode, deltaFlag) {
        ...
        if(deleteFlag){
            ...
            // fs.rmdirSync('./FetchedAuraCmp',{recursive: true, force: true})
            // fs.rmdirSync('./FetchedClasses',{recursive: true, force: true})
            // fs.rmdirSync('./FetchedLWC',{recursive: true, force: true})
            // fs.rmdirSync('./FetchedVFPage',{recursive: true, force: true})
            // fs.rmdirSync('./FetchedVFComponents',{recursive: true, force: true})
            // fs.rmdirSync('./Fetched_CustomLabels',{recursive: true, force: true})
            // fs.rmdirSync('./Fetched_Objects',{recursive: true, force: true})
            // fs.rmdirSync('./Fetched_Records',{recursive: true, force: true})
            // fs.rmdirSync('./Fetched_FieldSets',{recursive: true, force: true})
            // fs.rmdirSync('./Fetched_StaticResource',{recursive: true, force: true})
        }
        ...
        for (let orgName of ORG_NAMES) {
            try {
                ...
                for(let entity in codeToFetchParam) {
                    if(keysToIgnore.includes(entity)) continue;
                    let output = await this[entityFunctionMapping[entity]](codeToFetchParam[entity], orgName, conn, entity);
                    orgFetchReport[orgName][entity] = output.data.count;
                    enForceResponseData[orgName].push(output);
                }

                // let apexClassCount = await this.fetchApexClasses(codeToFetch['ApexClass'], orgName, conn, 'ApexClass');
                // let auraCmpCount = await this.fetchAuraComponents(codeToFetch['AuraComponent'], orgName, conn, 'AuraComponent');
                // let lwcCount = await this.fetchLWCComponents(codeToFetch['LWC'], orgName, conn, 'LWC');
                // let vfPageCount = await this.fetchVisualforcePages(codeToFetch['VFPage'], orgName, conn, 'VFPage');
                // let vfComponentCount = await this.fetchVisualforceComponents(codeToFetch['VFComponent'], orgName, conn, 'VFComponent');
                // let customLabelCount = await this.fetchCustomLabel(codeToFetch['CustomLabels'], orgName, conn, 'CustomLabels');
                // let objectCount = await this.fetchObjects(codeToFetch['Objects'], orgName, conn, 'Objects');
                // let objectRecordsCount = await this.fetchObjectRecords(codeToFetch['ObjectRecords'], orgName, conn, 'ObjectRecords');
                // let fieldSetsCount = await this.fetchFieldSets(codeToFetch['FieldSets'], orgName, conn, 'FieldSets');
                // let staticResourceCount = await this.fetchStaticResource(codeToFetch['StaticResource'], orgName, conn, 'StaticResource');

                // console.log('\n\n');
                // debug(`Report => Count of items fetched = `);
                // debug(`\tApex Class = ${apexClassCount}`);
                // debug(`\tAura Component = ${auraCmpCount}`);
                // debug(`\tLWC = ${lwcCount}`);
                // debug(`\tVF Page Count = ${vfPageCount}`);
                // debug(`\tVF Component Count = ${vfComponentCount}`);
                // debug(`\tCustom Label Count = ${customLabelCount}`);
                // debug(`\tObject Count = ${objectCount}`);
                // debug(`\tRecords Count = ${objectRecordsCount}`);
                // debug(`\tFieldSets Count = ${fieldSetsCount}`);
                // debug(`\tStatic Resource Count = ${staticResourceCount}`);
    ...
    getCreds(codeToFetchParam , orgName) {
        return Utils.getOrg(orgName);
    }

    processNames(codeToFetch){
        for(let key in codeToFetch) {
            if(keysToIgnore.includes(key)) continue;

            // console.log('###' , codeToFetch[key]);
            if(codeToFetch[key].names) 
                codeToFetch[key].names = codeToFetch[key].names.map(x => x.trim());
            if(codeToFetch[key].fileNames)
                codeToFetch[key].fileNames = codeToFetch[key].fileNames.map(x => x.trim());
        }
    }


    async fetchAuraComponents(auraComponents, orgName, conn, entityName) {
        console.log('\n');
		debug('Aura Components');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
        try {
    ...
    async fetchApexClasses(apexClasses, orgName, conn, entityName) {
        console.log('\n');
		debug('Apex Classes');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
    ...
    async fetchLWCComponents(lwcComponents, orgName, conn, entityName) {
        console.log('\n');
    ...
    async fetchVisualforcePages(vfPages, orgName, conn, entityName) {
        console.log('\n');
    ...
    async fetchVisualforceComponents(vfComponents, orgName, conn, entityName) {
        console.log('\n');
    ...
    async fetchCustomLabel(labelNames, orgName, conn, entityName) {
        console.log('\n');
		debug('Custom Label');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
    ...
    async fetchObjects(objectNames, orgName, conn, entityName) {
        console.log('\n');
		debug('Objects');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
    ...
    async fetchObjectRecords(objectNames, orgName, conn, entityName) {
        console.log('\n');
    ...
    async fetchFieldSets(fieldSets, orgName, conn, entityName) {
        console.log('\n');
		debug('FieldSets');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
    ...
    async fetchStaticResource(staticResources, orgName, conn, entityName) {
        console.log('\n');
    ...
    async fetchEmailTemplate(emailTemplates, orgName, conn, entityName) {
        console.log('\n');
		debug('EmailTemplate');
        let count = 0;
        let returnData = { count : 0 , entityName : entityName };
    ...
    async fetchCustomMetadataRecords(customMetadataRecords, orgName, conn, entityName) {
        console.log('\n');
    ...
    deltaComparison(codeToFetchParam, orgFetchReport){
        // return;
        ...
        for(let entity of entitiesFetched) {
            ...
            let superSetOfContents = new Set([...contents1 , ...contents2]);
    ...
    compareContents(superSet, path1, path2) {
        let exactMatch = [];
